...
export function searchVerses(query: string): BibleVerse[] {
  ...
  return getAllVerses().filter((verse) => {
    ...
    return text.includes(normalizedQuery) || book.includes(normalizedQuery) || reference.includes(normalizedQuery);
  });
}
...
export function getChapter(book: string, chapter: number): BibleVerse[] {
  return getAllVerses().filter((item) => item.book === book && item.chapter === chapter);
}
